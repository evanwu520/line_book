package com.example.linebook.service.impl;
...
public class BookServiceImpl implements BookService {
    ...
    public BookSearchResponse searchBooks(String title, String author, BookType bookType,
        ...
        List<BookSearch> bookSearches = books.getContent().stream().distinct().map(book -> {
            return new BookSearch(book, bookCopyRepository.countCopiesByLibrary(book));
        ...
        List<BookSearchResponse.book> bookResponseList =  bookSearches.stream().distinct().map(bookSearch -> {
            ...
            bookResponse.setBookCopies(
                    bookSearch.getBookCopies().stream()
                    .collect(Collectors.groupingBy(lb -> lb.getLibrary().getId()))
                    .values().stream()
                    .map(libraryBookCounts -> {
                        Map<String, Long> statusCount = libraryBookCounts.stream()
                                .collect(Collectors.groupingBy(
                                        lc -> lc.getStatus().name().toLowerCase(),
                                        Collectors.summingLong(LibraryBookCount::getCount)
                                ));

                        statusCount.putIfAbsent(BookCopyStatus.AVAILABLE.name().toLowerCase(), 0L);
                        statusCount.putIfAbsent(BookCopyStatus.LOANED.name().toLowerCase(), 0L);

                        Library library = libraryBookCounts.get(0).getLibrary();

                        return new BookSearchResponse.LibraryBookCount(
                                library.getId(),
                                library.getName(),
                                statusCount
                        );
